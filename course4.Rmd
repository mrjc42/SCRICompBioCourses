...
2. Set up your workspace by loading all the appropriate libraries you will need and setting your working directory.
...
setwd('/data/staylo/rnaseq/')
```

3. The DESeq2 package takes a table of counts. The column names are the individual sample names. The rownames are the gene names. So our first task is to load all the individual counts tables, merge them into a single counts table, and then make sure it is formatted properly.
```{r}
# Create vector of file paths for all gene_read_counts.tsv files
countsfiles<-list.files(path = 'expression', pattern = 'gene_read_counts.tsv', recursive = TRUE, full.names = TRUE)

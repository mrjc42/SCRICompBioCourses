...
Here you can see that the object is storing more than one element, namely each letter of the alphabet. This type of structure is called a __vector__. You will be tempted to think of a vector as a list. That's OK in the beginning, but not quite true. It's really a little more like an array: 



$$\mathbf{LETTERS} = \left[\begin{array}
{rrr}
'A' & 'B' & 'C' \\
'D' & 'E' & 'F' \\
'G' & 'H' & 'I'
...
An vector is basically a single object with many elements. Operations performed on the object are performed on all elements simultaneously.

Look back again at the output from `LETTERS`. Please notice a couple of things about this.  The 1st is that this is a vector of character elements.  But the 2nd is that there is a report at the start of each line to indicate which element number you are looking at...
